...
def download_kegg(path=None):
    """
    Downloads the latest version of KEGG (from REST API and TogoWS) via BioPython

    :param path: the path to write the downloaded data to
    :type path: str
    :return: the path of the written data
    """
...
def _download_lists(path,dbs=["reaction","compound"]):
    ...
    Lists are used to grab entries.
    """
...
def create_shutl_zips(path):
    """
    Writes compressed versions of all jsons found when walking the path

    :param path: the path containing the jsons
    :type path: str
    :return: Nothing
    """
